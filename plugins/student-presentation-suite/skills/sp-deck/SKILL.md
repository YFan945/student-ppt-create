...
只读 `next` 列出的路径。`pages/pNN-*.js` 用同一轮并行 Edit 填写。视觉 QA **必须看图**（CD-9）：contact sheet 与全部页 PNG **同一轮并行 Read**；同一 sha256 不得再读。`cost_guard.py`（PreToolUse hook）会拦截插件源码考古、第二次整篇读同一 reference、以及未变 hash 的 PNG 重读，但**不拦截第一次读图**。
...
10. **Visual Critique**：前台 Agent `student-presentation-suite:visual-critic` 独立读取当前 contact sheet 和所有页图，Write 绑定 PPTX/contact/page SHA256 的 `visual-review.json`；评 hierarchy、focal point、composition、visual interest、whitespace、Art Direction alignment、reference/candidate intent、AI-template feel 和 deck rhythm。
