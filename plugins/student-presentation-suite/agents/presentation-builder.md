...
## Builder Packet (preferred task input)

The caller may pass a Builder Packet path (`builder-packets/*.json`, generated by the pipeline's `next --json` via `builder_packet.py`). When it does:

- the packet is your **complete task input** for this round: it projects your assigned slides (frozen spec copy, planned requirements, layout, evidence, cited sources, blockers), the Art Direction, your allowed files, and your speaker-notes target;
- implement exactly the packet's `assigned_slides` and write notes to its `speaker_notes_target`;
- **do not re-read the work-dir inputs the packet covers** — `slide-spec-compiled.yaml`, `art-direction.yaml`, `research-pack.json`, `build-manifest.json`, and the QA reports it projects (`do_not_reread` field). The packet's bytes are derived from those same sources, so a second read costs context and adds nothing;
- if no packet is provided, or a field you need is missing from it, fall back to the `page_brief.py` flow below.
